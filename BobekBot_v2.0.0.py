...
def log_command(interaction: discord.Interaction, options: dict, was_successful: bool):
    ...
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            # Ensure user and guild are in their respective tables
            cursor.execute("INSERT OR IGNORE INTO users (user_id, username) VALUES (?, ?)",
                           (interaction.user.id, interaction.user.name))
            guild_id = interaction.guild.id if interaction.guild else None
            if guild_id:
                cursor.execute("INSERT OR IGNORE INTO guilds (guild_id, name) VALUES (?, ?)",
        ...
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [CMD] [{status}] User '{interaction.user.name}' ({interaction.user.id}) ran '/{command_name}' in '{guild_name}'. Options: {json.dumps(options)}")
...
async def on_ready():
    ...
    with sqlite3.connect(DB_FILE) as conn:
        cursor = conn.cursor()
        db_guilds = {row[0] for row in cursor.execute("SELECT guild_id FROM guilds").fetchall()}
        current_guilds = {guild.id for guild in client.guilds}

        # 1. Find guilds the bot was removed from while offline
        removed_guilds = db_guilds - current_guilds
        if removed_guilds:
            cursor.executemany("UPDATE guilds SET is_active = 0 WHERE guild_id = ?", [(gid,) for gid in removed_guilds])
            print(f"[SYSTEM] Marked {len(removed_guilds)} guild(s) as inactive (bot was removed while offline).")

        # 2. Find guilds the bot was added to while offline or re-joined
        added_guilds = current_guilds - db_guilds
        if added_guilds:
            added_guild_data = [(guild.id, guild.name, 1) for guild in client.guilds if guild.id in added_guilds]
            cursor.executemany("INSERT INTO guilds (guild_id, name, is_active) VALUES (?, ?, ?)", added_guild_data)
            print(f"[SYSTEM] Detected {len(added_guilds)} new guild(s) joined while offline.")

        # 3. Update existing guilds (name might have changed, or re-joined)
        existing_guilds = current_guilds.intersection(db_guilds)
        if existing_guilds:
            update_guild_data = [(guild.name, 1, guild.id) for guild in client.guilds if guild.id in existing_guilds]
            cursor.executemany("UPDATE guilds SET name = ?, is_active = ? WHERE guild_id = ?", update_guild_data)
...
async def on_guild_join(guild: discord.Guild):
    ...
    with sqlite3.connect(DB_FILE) as conn:
        conn.execute("INSERT OR REPLACE INTO guilds (guild_id, name, is_active) VALUES (?, ?, 1)",
                     (guild.id, guild.name))
        conn.commit()
    print(
...
async def on_guild_remove(guild: discord.Guild):
    ...
    with sqlite3.connect(DB_FILE) as conn:
        conn.execute("UPDATE guilds SET is_active = 0 WHERE guild_id = ?", (guild.id,))
        conn.commit()
    print(
...
@app_commands.describe(
    ...
    hide_output="If True, the bot's reply will also be hidden. Default: False"
)
async def owner_command(interaction: discord.Interaction, command: str, hide_output: bool = False):
    if interaction.user.id not in OWNER_IDS:
        log_command(interaction, {'command': command, 'hide_output': hide_output}, was_successful=False)
        await interaction.response.send_message("You do not have permission to use this command.", ephemeral=True)
        return

    await interaction.response.defer(ephemeral=hide_output)
    parts = command.strip().split()
    subcommand = parts[0].lower() if parts else ''
    args = parts[1:]
    log_command(interaction, {'command': command, 'hide_output': hide_output}, was_successful=True)
    ...
    if subcommand == 'help':
        ...
        embed.add_field(name="`repeat <text>`", value="Repeats the text you provide.", inline=False)
        embed.set_footer(text="All commands are run via /owner command: <subcommand>")
        await interaction.followup.send(embed=embed, ephemeral=hide_output)
    elif subcommand == 'shutdown':
        await interaction.followup.send("Bot is shutting down.", ephemeral=hide_output)
        await client.close()
        ...
        await interaction.followup.send(text_to_repeat, ephemeral=hide_output)
    else:
